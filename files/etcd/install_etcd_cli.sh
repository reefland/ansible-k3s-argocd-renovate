...
if [ "$(id -u)" -ne 0 ]; then
  echo
  echo "* ERROR: ROOT privilege required to access ETCD certificates and install ETCD CLI."
  echo
  exit 1
fi

#K3s older than ~v1.25 uses port 2379
ETCD_PORT=2382

ETCD_VER="v$(curl -L --cacert /var/lib/rancher/k3s/server/tls/etcd/server-ca.crt --cert /var/lib/rancher/k3s/server/tls/etcd/server-client.crt --key /var/lib/rancher/k3s/server/tls/etcd/server-client.key -s https://127.0.0.1:${ETCD_PORT}/version | cut -d \" -f 4)"
